...
export async function getMultipleCardData(orderIds: string[]) {
  try {
    ...
    // Verify user is a super printer
    const user = await prisma.user.findUnique({
      where: { id: userId, role: "superPrinter" },
    });

    if (!user) {
      return {
        status: false,
        message: "Unauthorized - Super Printer access required",
      };
    }
...
export async function multiPrint(ids: string[]) {
  try {
    ...
    // Verify user is a super printer
    const user = await prisma.user.findUnique({
      where: { id: adminId, role: "superPrinter" },
    });
    ...
    // Mark multiple orders as printed (super printer can print from any station)
...
export async function multiApprove(ids: string[]) {
  try {
    ...
    // Update multiple orders to APPROVED
...
export async function multiReject(ids: string[]) {
  try {
    ...
    // Update multiple orders to REJECTED
    const result = await prisma.order.updateMany({
      where: {
        id: { in: ids },
      },
